...
const Orders = () => {
  ...
  // Fetch all orders for this user and flatten into per-item rows
  const loadOrderData = async () => {
    try {
      ...
      if (res.data.success) {
        // Flatten: each order item becomes its own row with order-level metadata
        const allOrders = [];
          ...
          >
            ...
            <div className="md:w-1/2 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <p className={`text-sm font-medium px-3 py-1 rounded-full ${statusColor(item.status)}`}>
                  {item.status}
                </p>
              </div>
              <button
